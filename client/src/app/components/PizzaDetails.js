...
const PizzaDetails = ({ pizza, setModal }) => {
  ...
  useEffect(() => {
    if (additionalTopping.length > 0) {
      const toppingPrice = additionalTopping.reduce((acc, curr) => acc + curr.price, 0);
    ...
    <div className="flex flex-col lg:flex-row lg:gap-x-8 h-full md:p-8">
      ...
      <div className="lg:flex-1 flex justify-center items-center">
        ...
        <div className="max-w-[300px] lg:max-w-none mt-6 lg:mt-0">
          <Image width={450} height={450} alt='' src={pizza.image} priority={1} className="mx-auto relative" />
        </div>
      </div>
      {/* details */}
